...
import com.zsxfa.cloud.core.config.ConfigManager;
...
import com.zsxfa.cloud.core.pojo.dto.file.FileModel;
import com.zsxfa.cloud.core.pojo.dto.office.CreateOfficeFileDTO;
import com.zsxfa.cloud.core.pojo.dto.office.EditOfficeFileDTO;
import com.zsxfa.cloud.core.pojo.dto.office.PreviewOfficeFileDTO;
...
@RestController
@Api(tags = "Onlyoffice文件操作接口")
@Slf4j
@RequestMapping("/api/core/office")
public class OfficeController {
    ...
    @ApiOperation("创建office文件")
    @ResponseBody
    @PostMapping(value = "/createofficefile")
    public R createOfficeFile(@RequestBody CreateOfficeFileDTO createOfficeFileDTO, @RequestHeader("token") String token) {
        try{
    ...
    @ApiOperation("预览office文件")
    @PostMapping(value = "/previewofficefile")
    @ResponseBody
    public R previewOfficeFile(HttpServletRequest request, @RequestBody PreviewOfficeFileDTO previewOfficeFileDTO, @RequestHeader("token") String token) {
        ...
        try {
            ...
            return R.ok().data("jsonObject",jsonObject);
        } catch (Exception e) {
            log.error(e.getMessage());
            return R.error();
        }
    ...
    @ApiOperation("编辑office文件")
    @ResponseBody
    @PostMapping(value = "/editofficefile")
    public R editOfficeFile(HttpServletRequest request, @RequestBody EditOfficeFileDTO editOfficeFileDTO, @RequestHeader("token") String token) {
        RestResult<Object> result = new RestResult<>();
        ...
        if ("2".equals(status) || "6".equals(status)) {
            ...
            if("edit".equals(type)){//修改报告
                ...
                try {
                    ...
                    lambdaUpdateWrapper
                            .set(File::getIdentifier, md5Str)
                            .set(File::getFileSize, Long.valueOf(fileLength))
